...
def main():
    ...
    trash_labels = {'bio': 'Biodegradable', 'nbio': 'Non-Biodegradable', 'hazardous': 'Hazardous'}
    ...
    while True:
        ...
        if msvcrt.kbhit():
            ...
            if choice == 'q':
                ...
                # Mixed: tilt-only command and record 'mixed'
                ser.write(b"tdeg\n")
                print("\n🔄 Tilting D9 to 30 degrees (Mixed)...")
                time.sleep(0.1)
                while ser.in_waiting:
                    response = ser.readline().decode().strip()
                    if response:
                        print(f"🟢 Arduino Response: {response}")
                # Record the mixed operation
                try:
                    requests.post(
                        f"http://{ip_address}/GoSort_Web/gs_DB/record_sorting.php",
                        json={
